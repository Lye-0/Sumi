...
添付は1ファイルずつ行い、各試行を180秒で打ち切り、最大3回試します。全ファイルのサイズ・SHA-256・アップロード状態がGitHub側の記録と一致した場合だけ公開します。失敗した場合は下書きを残します。
添付中の通信失敗なら、Actionsの「Re-run failed jobs」でビルド済みの配布物を使って再開できます。再実行は元のタグのワークフローを使うため、main上の修正は反映されません。修正版のワークフローを使用する場合は新しいバージョンタグを作成してください。
