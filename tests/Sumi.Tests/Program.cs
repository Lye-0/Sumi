...
var combined = stock.WithFinalImage([4]);
Check(combined.Length == 3 && combined[0][0] == 1 && combined[1][0] == 2 && combined[2][0] == 4 && stock.Count == 2, "final capture appended after stock without mutating retained images");
Check(new ImageStock().WithFinalImage([7]).Single()[0] == 7, "empty stock sends only final capture");
